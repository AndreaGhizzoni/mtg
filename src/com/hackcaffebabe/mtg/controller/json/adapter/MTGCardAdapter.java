...
public class MTGCardAdapter implements JsonSerializer<MTGCard>, JsonDeserializer<MTGCard>
{
	...
	public JsonElement serialize(MTGCard c, Type arg1, JsonSerializationContext arg2){
		...
		result.add( JSON_TAG_PRIMARY_EFFECT, new JsonPrimitive( c.getPrimaryEffect() == null ? ""
				: c.getPrimaryEffect() ) );
	...
	public MTGCard deserialize(JsonElement json, Type arg1, JsonDeserializationContext context)
		...
		try {
			MTGCard c = context.deserialize( json,
					Class.forName( "com.hackcaffebabe.mtg.model." + type ) );
			...
			if(!type.equals( "Land" )) {
				ManaCost cos = context.deserialize(
						json.getAsJsonObject().get( JSON_TAG_MANA_COST ),
						Class.forName( "com.hackcaffebabe.mtg.model.card.ManaCost" ) );
				if(c instanceof Creature) {
					Strength s = context.deserialize( json.getAsJsonObject()
							.get( JSON_TAG_STRENGTH ), Class
							.forName( "com.hackcaffebabe.mtg.model.card.Strength" ) );
					...
					for(JsonElement i: json.getAsJsonObject().get( JSON_TAG_PLANES_ABILITY )
							.getAsJsonArray()) {
